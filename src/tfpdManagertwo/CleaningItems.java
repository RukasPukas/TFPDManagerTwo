...
public class CleaningItems 
{
	...
	CleaningItems(String enteredUserName, String enteredPassword, Boolean adminStatus, Boolean superAdminStatus, String userEmail)
	{
		
		
		cleaningFrame = new JFrame("Cleaning Item Requests");
		cleaningFrame.setSize(911,791);
		cleaningFrame.setDefaultCloseOperation(JFrame.DO_NOTHING_ON_CLOSE); 
		cleaningFrame.setLocationRelativeTo(null);
		cleaningFrame.setResizable(false);
		
		
		ImageIcon icon = new ImageIcon("TroyIcon.png");
		Image img = icon.getImage();
		cleaningFrame.setIconImage(img);
		

		panel=new JPanel();
		panel.setBackground(Color.DARK_GRAY);
		panel.setLayout(null);
		cleaningFrame.getContentPane().add(panel);
		
		JLabel cleaningLabelPrompt = new JLabel("Cleaning Supplies");
		cleaningLabelPrompt.setForeground(Color.WHITE);
		cleaningLabelPrompt.setFont(new Font("Monospaced", Font.BOLD, 30));
		cleaningLabelPrompt.setBounds(10, 14, 357, 71);
		panel.add(cleaningLabelPrompt);
		
		JComboBox comboBox = new JComboBox();
		comboBox.setBounds(33, 156, 245, 49);
		panel.add(comboBox);
		
		//<------------------------------------------------------This is where cleaningList.txt is read------------------------------------->
	       String filePath = "ItemLists/cleaningitems.txt";
	       ...
	       try (BufferedReader reader = new BufferedReader(new FileReader(filePath))) {
		            ...
		            System.out.println("Cleaining Items List loaded successfully");
		        } catch (IOException e) {
		        	JOptionPane.showMessageDialog(null, "An Error occurred retrieving the Cleaning Items List.");
		        	System.out.println("An Error occurred retrieving the Cleaning Items List.");
		            e.printStackTrace();
		        }
			
			//<--------------------------------------------------------End reading of cleaningList.txt------------------------------------------->
			...
			public void actionPerformed(ActionEvent e) {
				...
				else 
				{
					    ...
					    else 
					    {
			       			...
			       			//<-------------------------------Update cleaning items file--------------------------------------->
			       			File cleaningItemsFile = new File("ItemLists/cleaningitems.txt");
			       			...
			       			try {
			       			    ...
			       			    try (BufferedWriter writer = new BufferedWriter(new FileWriter(cleaningItemsFile))) {
			       			        ...
			       			        System.out.println("Cleaning Items File updated successfully");
			       			    }
			       			} catch (IOException ex) {
			       			    JOptionPane.showMessageDialog(null, "Issue updating the cleaning items file.");
			       			    ex.printStackTrace();
			       			}

			       			System.out.println("The selected item was added to the cleaning item list:  " + newItemToAdd);
			       			JOptionPane.showMessageDialog(null, "Item was successfully added to the list, the page will now refresh.");
			       			cleaningFrame.dispose();
			...
			public void actionPerformed(ActionEvent e) {
				...
				else
				{
					...
					else
					{
						...
						tempFile = new File("ItemLists/tempcleaningfile.txt");
		...
		submitButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
      		...
      		String checkPath = "EmailLists/cleaningemail.txt";
				...
				else
				{
					...
					String locateUserNames = "EmailLists/cleaningEmail.txt"; 
					           ...
					           try (BufferedReader fileReader = new BufferedReader(new FileReader(userToBeEmailedFile))) 
					           {
				                    ...
				                    while ((finalEmail = fileReader.readLine()) != null) 
				                    {
										...
										try {
										    ...
										    String htmlContent = "<H2>"+ enteredUserName+ " has sent  a cleaning supplies request from TFPD Manager.</H2>"
										    ...
										    message.setSubject("Cleaning Supplies Request From "+ enteredUserName);
					...
					//<-------------------------------End Emailing process--------------------------------->
	       			...
	       			try 
	       			{
	       			 ...
	       			 catch (IOException ex) 
	       			 {
	       			    ...
	       			    String newContent = "\n" + enteredUserName + " requested the following cleaning supplies:\n" + tempFileContent+ "on " + todayDate +"\n";;
		...
		emailRecipients.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				new CleaningRecipients( enteredUserName,  enteredPassword,  adminStatus,  superAdminStatus,  userEmail, cleaningFrame);
				cleaningFrame.setEnabled(false);
			}
		});
